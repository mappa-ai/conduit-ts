...
const InterpretableScoreSchema = require_transport.object({
	...
	normalized: require_transport.number().gte(0).lte(1).describe("Normalized score from 0 to 1").transform((n) => Math.round(n * 100) / 100)
});
const LayeredLenses = require_transport.object({
	archetypal_story: require_transport.object({ pmai: require_transport.object({
	...
	cognitive_override: require_transport.object({ crt: require_transport.object({ crt_score: InterpretableScoreSchema }) }),
	flexibility_mask: require_transport.object({ sms: require_transport.object({ self_monitoring: InterpretableScoreSchema }) }),
	interpersonal_stance: require_transport.object({ ipip_ipc: require_transport.object({
	...
	motivational_weights: require_transport.object({ pvq: require_transport.object({
	...
	temperamental_reactivity: require_transport.object({
		bis_bas: require_transport.object({
		...
		erq: require_transport.object({
			cognitive_reappraisal: InterpretableScoreSchema,
			expressive_suppression: InterpretableScoreSchema
		})
	}),
	trait_baseline: require_transport.object({
		hexaco: require_transport.object({
			agreeableness: require_transport.object({
				facet: InterpretableScoreSchema,
				factors: require_transport.object({
			...
			conscientiousness: require_transport.object({
				facet: InterpretableScoreSchema,
				factors: require_transport.object({
			...
			emotionality: require_transport.object({
				facet: InterpretableScoreSchema,
				factors: require_transport.object({
			...
			extraversion: require_transport.object({
				facet: InterpretableScoreSchema,
				factors: require_transport.object({
			...
			honesty_humility: require_transport.object({
				facet: InterpretableScoreSchema,
				factors: require_transport.object({
			...
			openness_to_experience: require_transport.object({
				facet: InterpretableScoreSchema,
				factors: require_transport.object({
...
const normalizedScore = require_transport.number().min(0).max(1);
const mappaToInterpretable = require_transport.object({
...
const rawTraitMap = require_transport.object({
	achievement: normalizedScore,
...
const layeredLensesResponse = require_transport.object({
	model_info: require_transport.object({ checkpoint: require_transport.string() }),
	traits: rawTraitMap
});

//#endregion
//#region src/internal-resources/behavior-maps.ts
const behaviorMapExportResponse = require_transport.object({
	behaviorMap: LayeredLenses,
	createdAt: require_transport.datetime(),
	entityId: require_transport.string().min(1),
...
var InternalBehaviorMapsResource = class {
	...
	async get(entityId, opts) {
		...
		return require_transport.parseRes(behaviorMapExportResponse, (await this.transport.request({
			method: "GET",
...
var InternalConduit = class {
	...
	constructor(options) {
		...
		const behaviorMaps = new InternalBehaviorMapsResource(new require_transport.Transport({
...
exports.InternalConduit = InternalConduit;
